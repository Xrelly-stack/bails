...
export const generateWAMessageContent = async (message, options) => {
  ...
  if (hasNonNullishProperty(message, 'text')) {
  ...
  } else if (hasNonNullishProperty(message, 'botInvoke')) {
    m.botInvokeMessage = {
  ...
  } else if (hasNonNullishProperty(message, 'product') || hasNonNullishProperty(message, 'productMessage')) {
    const isProductMessageAlias = hasNonNullishProperty(message, 'productMessage') && !hasNonNullishProperty(message, 'product');
    const input = isProductMessageAlias ? message.productMessage : message.product;
    const product = input.product ?? input;
    const productImage = product.productImage ?? input.thumbnail;
    if (!productImage) {
      throw new Boom('productMessage.productImage or productMessage.thumbnail is required', {
        statusCode: 400
      });
    }
    const {
    ...
    const productPayload = {
      ...(isProductMessageAlias ? input : message),
      product: {
        ...product,
        productImage: imageMessage
      }
    };
    delete productPayload.productMessage;
    delete productPayload.thumbnail;
    delete productPayload.productImage;
    m.productMessage = WAProto.Message.ProductMessage.create(productPayload);
