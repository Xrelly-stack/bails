...
export const makeSocket = config => {
  ...
  const syncDisabled = PROCESSABLE_HISTORY_TYPES.map(syncType => config.shouldSyncHistoryMessage({
    syncType
  ...
  const pnFromLIDUSync = async jids => {
    ...
    const results = await executeUSyncQuery(usyncQuery);
    if (results) {
      return results.list.filter(a => !!a.lid).map(({
        lid,
        id
      }) => ({
        pn: id,
  ...
  const uploadPreKeys = async (count = MIN_PREKEY_COUNT) => {
    ...
    uploadPreKeysPromise = Promise.race([uploadLogic(0), new Promise((_, reject) => setTimeout(() => reject(new Boom('Pre-key upload timeout', {
      statusCode: 408
    })), UPLOAD_TIMEOUT))]);
    try {
      await uploadPreKeysPromise;
  ...
  const startKeepAliveRequest = () => keepAliveReq = setInterval(() => {
    ...
    if (diff > keepAliveIntervalMs + 5000) {
      void end(new Boom('Connection was lost', {
  ...
  const logout = async msg => {
    ...
    void end(new Boom(msg || 'Intentional Logout', {
      statusCode: DisconnectReason.loggedOut
    }));
  };
  const requestPairingCode = async (phoneNumber, customPairingCode) => {
    // Tunggu handshake WebSocket selesai sebelum mengirim companion_hello.
    // Ini mencegah status 428 / Connection Closed saat pairing code diminta terlalu cepat.
  ...
  ws.on('close', () => void end(new Boom('Connection Terminated', {
    statusCode: DisconnectReason.connectionClosed
  })));
  ws.on('CB:xmlstreamend', () => void end(new Boom('Connection Terminated by Server', {
    statusCode: DisconnectReason.connectionClosed
  })));
  ws.on('CB:iq,type:set,pair-device', async stanza => {
    ...
    const genPairQR = () => {
      ...
      if (!refNode) {
        void end(new Boom('QR refs attempts ended', {
  ...
  ws.on('CB:ib,,downgrade_webclient', () => {
    void end(new Boom('Multi-device beta not joined', {
