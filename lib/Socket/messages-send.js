...
export const makeMessagesSocket = config => {
  ...
  const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
    ...
    if (result) {
      ...
      if (lidResults.length > 0) {
        ...
        await signalRepository.lidMapping.storeLIDPNMappings(lidResults.map(a => ({
          lid: a.lid,
          pn: a.id
        })));
        try {
  ...
  const createParticipantNodes = async (recipientJids, message, extraAttrs, dsmMessage) => {
    ...
    const patchedMessages = Array.isArray(patched) ? patched : recipientJids.map(jid => ({
      recipientJid: jid,
      message: patched
    }));
    ...
    const encryptionPromises = patchedMessages.map(async ({
      recipientJid: jid,
  ...
  const relayMessage = async (jid, message, {
    ...
    if (isRetryResend) {
      ...
      const {
        user,
        device
      } = jidDecode(retryParticipants.jid);
      devices.push({
        user,
        device,
    ...
    await authState.keys.transaction(async () => {
      ...
      if (isGroupOrStatus && !isRetryResend) {
        ...
        for (const {
          ...
          if (isMe) meRecipients.push(jid);else otherRecipients.push(jid);
        ...
        }] = await Promise.all([createParticipantNodes(meRecipients, meMsg || message, extraAttrs), createParticipantNodes(otherRecipients, message, extraAttrs, meMsg)]);
      ...
      if (isRetryResend) {
        ...
        const encodedMessageToSend = isMe ? encodeWAMessage({
          deviceSentMessage: {
            destinationJid,
          ...
          if (isLidUser(lidJid)) {
            ...
            if (nctSalt?.length) {
              ...
              logger.debug({
                jid: destinationJid,
                lid: lidJid
              }, 'attached cstoken (nct fallback)');
            } else {
              logger.debug({
                jid: destinationJid
              }, 'cstoken requested but no nct salt stored yet');
            }
          } else {
            logger.debug({
              jid: destinationJid
            }, 'cstoken requested but recipient has no LID');
          }
        } catch (err) {
          logger.debug({
            jid: destinationJid,
            err: err?.message
          }, 'cstoken attach failed');
        }
      }
      ...
      while (inner && typeof inner === "object") {
        const entry = Object.entries(inner).find(([, v]) => v && typeof v === "object" && (v.message || v.templateMessage || v.buttonsMessage || v.interactiveMessage));
        if (!entry) break;
        const [, value] = entry;
        if (value.message) {
          hasWrapper = true;
  ...
  const getButtonArgs = message => {
    if (message.templateMessage) {
      ...
      return {
        v: '2',
        type: proto.ListMessage.ListType[type].toLowerCase()
      };
    } else {
      return {};
    }
  ...
  return {
    ...
    /**
     * Send a message to members of a group using the xzcbailz-compatible API.
     * Defaults to all members; set onlyMember=true to target non-admin members only.
     */
    sendMessageMembers: async (groupJid, content, options = {}) => {
      if (!isJidGroup(groupJid)) {
        throw new Error('sendMessageMembers requires a group JID ending in @g.us');
      }
      const {
        delayMs = 1500,
        onlyMember = false,
        ...
        cachedGroupMetadata: customGroupMetadata
      } = options;
      const metadata = customGroupMetadata
        ? await customGroupMetadata(groupJid)
        : await groupMetadata(groupJid);
      const participants = Array.isArray(metadata?.participants) ? metadata.participants : [];
      const selected = onlyMember
        ? participants.filter(participant => !participant.admin).map(participant => participant.id)
        : participants.map(participant => participant.id);
      const meId = authState.creds.me?.id;
      const recipients = selected.filter(id => id && (!meId || !areJidsSameUser(id, meId)));
      const results = [];
      for (let index = 0; index < recipients.length; index++) {
        const recipient = recipients[index];
        try {
          const fullMsg = await generateWAMessage(recipient, content, {
            logger,
            userJid: meId,
            quoted,
            upload: waUploadToServer,
            messageId: baseMessageId ? `${baseMessageId}_${index}` : undefined
          });
          await relayMessage(recipient, fullMsg.message, { messageId: fullMsg.key.id });
          results.push({ jid: recipient, ok: true, messageId: fullMsg.key.id });
        } catch (error) {
          logger.warn({ jid: recipient, error }, 'sendMessageMembers failed for participant');
          results.push({ jid: recipient, ok: false, error: error?.message || String(error) });
        }
        if (delayMs > 0 && index < recipients.length - 1) {
          await new Promise(resolve => setTimeout(resolve, delayMs));
        }
      }
      return { groupJid, total: recipients.length, results };
    },
    /**
     * Portable table sender compatible with xzcbailz's sendTable signature.
     * It uses a normal monospace text message instead of undocumented protobuf fields.
     */
    sendTable: async (jid, title, headers = [], rows = [], quoted, options = {}) => {
      ...
      const normalizedRows = Array.isArray(rows)
        ? rows.map(row => Array.isArray(row) ? row.map(value => String(value ?? '')) : [String(row ?? '')])
        : [];
      const width = Math.max(normalizedHeaders.length, ...normalizedRows.map(row => row.length), 0);
      const columnWidths = Array.from({ length: width }, (_, column) => Math.max(
        String(normalizedHeaders[column] ?? '').length,
        ...normalizedRows.map(row => String(row[column] ?? '').length),
        1
      ));
      const formatRow = row => `| ${Array.from({ length: width }, (_, column) => String(row[column] ?? '').padEnd(columnWidths[column])).join(' | ')} |`;
      const divider = `|-${columnWidths.map(size => '-'.repeat(size)).join('-|-')}-|`;
      const lines = [];
      if (title) lines.push(String(title));
      if (width) {
        lines.push(formatRow(normalizedHeaders));
        lines.push(divider);
        for (const row of normalizedRows) lines.push(formatRow(row));
      }
      const tableText = ['```', ...lines, '```'].join('\\n');
      return this.sendMessage(jid, { text: tableText }, { ...options, quoted });
    },
    ...
    /**
     * Convenience helper for sending a WhatsApp product message.
     * The low-level payload remains available through sendMessage({ productMessage }).
     */
    productMessage: async function (jid, product = {}, options = {}) {
      const payload = product?.productMessage || product?.product
        ? product
        : { productMessage: product };
      return this.sendMessage(jid, payload, options);
    },
