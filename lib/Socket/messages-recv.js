...
export const makeMessagesRecvSocket = config => {
    ...
    await authState.keys.transaction(async () => {
      ...
      if (retryCount > 1 || forceIncludeKeys || shouldRecreateSession) {
        ...
        content.push({
          ...
          }, xmppPreKey(key, +keyId), xmppSignedPreKey(signedPreKey), {
            tag: 'device-identity',
            attrs: {},
  ...
  const handleEncryptNotification = async node => {
    ...
    if (from === S_WHATSAPP_NET) {
      ...
      logger.debug({
        node
      }, 'recv encrypt notification (identity change)');
    }
  };
  ...
  const processNotification = async node => {
    ...
    switch (nodeType) {
      ...
      case 'link_code_companion_reg':
        ...
        authState.creds.advSecretKey = Buffer.from(hkdf(identityPayload, 32, {
          info: 'adv_secret'
        })).toString('base64');
        await query({
          tag: 'iq',
  ...
  const handleMessage = async node => {
    ...
    try {
            ...
            requestPlaceholderResend(cleanKey, msgData).then(requestId => {
              if (requestId && requestId !== 'RESOLVED') {
                ...
                ev.emit('messages.update', [{
                  key: msg.key,
                  update: {
                    messageStubParameters: [NO_MESSAGE_FOUND_ERROR_TEXT, requestId]
                  }
                }]);
              }
            }).catch(err => {
      ...
      if (!acked) {
        await sendMessageAck(node, NACK_REASONS.UnhandledError).catch(ackErr => logger.error({
          ackErr
