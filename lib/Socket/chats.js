...
export const makeChatsSocket = config => {
    ...
    return {
      onMutation(mutation) {
        ...
        if (nctSalt?.length) {
          authState.keys.set({
            'nct-salt': {
              default: Buffer.from(nctSalt)
            }
          }).catch(err => logger?.debug({
            err: err?.message
          }, 'failed to store nct salt'));
        }
      }
    };
  };
          ...
          try {
            if (snapshot) {
              const {
                state: newState,
                mutationMap
              } = await decodeSyncdSnapshot(name, snapshot, getCachedAppStateSyncKey, initialVersionMap[name], appStateMacVerification.snapshot, logger);
            ...
            if (patches.length) {
              const {
                state: newState,
                mutationMap
              } = await decodePatches(name, patches, states[name], getCachedAppStateSyncKey, config.options, initialVersionMap[name], logger, appStateMacVerification.patch);
              await authState.keys.set({
                'app-state-sync-version': {
                  [name]: newState
                }
  ...
  const presenceSubscribe = async toJid => {
    ...
    const tcTokenContent = isUserJid ? await buildTcTokenFromJid({
      authState,
      jid: normalizedToJid,
      getLIDForPN
    }) : undefined;
  ...
  const appPatch = async patchCreate => {
    ...
    if (!myAppStateKeyId) {
      throw new Boom('App state key not present!', {
        statusCode: 400
      });
    }
    let initial;
  ...
  const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
    ...
    const shouldProcessHistoryMsg = ignoreOfflineMessages ? false : historyMsg ? shouldSyncHistoryMessage(historyMsg) && PROCESSABLE_HISTORY_TYPES.includes(historyMsg.syncType) : false;
